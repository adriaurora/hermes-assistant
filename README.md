...
- 🔒 **Private by design**: credentials are encrypted with AndroidKeyStore AES-256-GCM,
  backups are disabled, and no third-party voice/cloud providers are involved.
...
Notifications require Firebase. A `google-services.json` must exist in the project
matching the applicationId `io.github.adriaurora.hermesassistant` (the config is
not committed to the repo). When a matching config is present the FCM capability
