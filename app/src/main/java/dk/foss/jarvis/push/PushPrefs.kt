...
class PushPrefs internal constructor(private val store: DataStore<Preferences>) : PushState, TransportChoiceStore {
    ...
    suspend fun protocol()=store.data.first()[Keys.PUSH_PROTOCOL]?.let { runCatching { PushProtocol.valueOf(it) }.getOrNull() } ?: PushProtocol.LEGACY
    suspend fun setProtocol(v:PushProtocol){store.edit{it[Keys.PUSH_PROTOCOL]=v.name}}
    ...
    suspend fun probeResult()=store.data.first()[Keys.PUSH_PROBE_RESULT]?.let{runCatching{PushTransport.valueOf(it)}.getOrNull()}
    suspend fun setProbeResult(t:PushTransport?,nowMs:Long?=null){store.edit{if(t==null)it.remove(Keys.PUSH_PROBE_RESULT)else it[Keys.PUSH_PROBE_RESULT]=t.name;if(nowMs!=null)it[Keys.PUSH_PROBE_AT]=nowMs}}
    override suspend fun choice()=store.data.first()[Keys.PUSH_TRANSPORT]
    override suspend fun lastProbe()=probeResult()
    override suspend fun lastProbeAt()=store.data.first()[Keys.PUSH_PROBE_AT]
    override suspend fun recordProbe(t:PushTransport,nowMs:Long){setProbeResult(t,nowMs)}
}
