...
class FcmRevokeCleanupTest {
 ...
 private fun makeSettingsStore(): SettingsStore { val ds=PreferenceDataStoreFactory.create{File(tmp.newFolder(),"settings.preferences_pb")}; return SettingsStore(ds, SecureStore(CleanupCipher(),CleanupBlobs())) }
 @Test fun `success with pending clear purges every stored copy of the credential`()=runBlocking { val (p,s,r)=make();r.save("dev","tok","http://a","k");s.saveToken("bearer");p.setPendingRevoke(true);p.setPendingCredentialClear(true);FcmRevokeCleanup.onComplete(r,s,p,makeSettingsStore()){};assertNull(s.loadToken());assertNull(s.loadPushApiKey());assertNull(s.loadDeviceId());assertFalse(p.isPendingRevoke());assertFalse(p.isPendingCredentialClear());assertEquals(FcmRegistrationState.DISABLED,p.registrationState.first()) }
 @Test fun `success without pending clear keeps the bearer principal`()=runBlocking { val (p,s,r)=make();r.save("dev","tok","http://a","k");s.saveToken("bearer");p.setPendingRevoke(true);FcmRevokeCleanup.onComplete(r,s,p,makeSettingsStore()){};assertEquals("bearer",s.loadToken());assertFalse(p.isPendingRevoke()) }
 @Test fun `completion re-registers when push was re-enabled during the revoke`()=runBlocking { val (p,s,r)=make();r.save("dev","tok","http://a","k");p.enable();p.setPendingRevoke(true);var called=false;FcmRevokeCleanup.onComplete(r,s,p,makeSettingsStore()){called=true;p.setRegistrationState(FcmRegistrationState.REGISTERING)};assertTrue(called);assertEquals(FcmRegistrationState.REGISTERING,p.registrationState.first()) }
}